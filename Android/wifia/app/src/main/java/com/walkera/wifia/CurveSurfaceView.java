...
public class CurveSurfaceView extends SurfaceView implements Callback {
    ...
    public CurveSurfaceView(Context context) {
        ...
        setBackgroundResource(R.drawable.background0);
    }
    ...
    public static void init() {
        f1w = ThemeSurfaceView.f15w;
        f0h = ThemeSurfaceView.f14h;
        yhalf = f0h / 2;
        xhalf = f1w / 2;
        xleft = 0;
        xright = f1w;
        ytop = 0;
        ybottom = f0h;
        scales = ThemeSurfaceView.scales;
        if (f1w < 320 || f0h < 320) {
            fix = 20;
            w0 = 100;
        } else {
        ...
        ch_en[0] = RunSurfaceView.pointLFlag;
        ch_en[1] = RunSurfaceView.point1Flag;
        ch_en[2] = RunSurfaceView.point2Flag;
        ch_en[3] = RunSurfaceView.pointMFlag;
        ch_en[4] = RunSurfaceView.point3Flag;
        ch_en[5] = RunSurfaceView.point4Flag;
        ch_en[6] = RunSurfaceView.pointHFlag;
        ch_out[0] = (RunSurfaceView.pointLL - 50.0f) * 2.0f;
        ch_out[1] = (RunSurfaceView.point11 - 50.0f) * 2.0f;
        ch_out[2] = (RunSurfaceView.point22 - 50.0f) * 2.0f;
        ch_out[3] = (RunSurfaceView.pointMM - 50.0f) * 2.0f;
        ch_out[4] = (RunSurfaceView.point33 - 50.0f) * 2.0f;
        ch_out[5] = (RunSurfaceView.point44 - 50.0f) * 2.0f;
        ch_out[6] = (RunSurfaceView.pointHH - 50.0f) * 2.0f;
        ch_ens[0] = RunSurfaceView.pointLSFlag;
        ch_ens[1] = RunSurfaceView.point1SFlag;
        ch_ens[2] = RunSurfaceView.point2SFlag;
        ch_ens[3] = RunSurfaceView.pointMSFlag;
        ch_ens[4] = RunSurfaceView.point3SFlag;
        ch_ens[5] = RunSurfaceView.point4SFlag;
        ch_ens[6] = RunSurfaceView.pointHSFlag;
        ch_outs[0] = RunSurfaceView.pointLLS;
        ch_outs[1] = RunSurfaceView.point11S;
        ch_outs[2] = RunSurfaceView.point22S;
        ch_outs[3] = RunSurfaceView.pointMMS;
        ch_outs[4] = RunSurfaceView.point33S;
        ch_outs[5] = RunSurfaceView.point44S;
        ch_outs[6] = RunSurfaceView.pointHHS;
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        int n;
        super.onDraw(canvas);
        Log.w("CurveSurfaceView", "onDraw w = " + f1w);
        Log.w("CurveSurfaceView", "onDraw w0= " + w0);
        paint.setColor(Color.RED);
        canvas.drawLine((float) (f1w / 2), (float) ((f0h + RunSurfaceView.arrow) / 2), (float) f1w, (float) ((f0h + RunSurfaceView.arrow) / 2), paint);
        canvas.drawLine((float) (f1w - w0), (float) RunSurfaceView.arrow, (float) (f1w - w0), (float) f0h, paint);
        paint.setColor(Color.GREEN);
        canvas.drawText("Input", ((float) f1w) - (40.0f * scales), (float) ((f0h / 2) + RunSurfaceView.arrow), paint);
        canvas.drawText("Output", (float) (f1w - w0), ((float) RunSurfaceView.arrow) + (18.0f * scales), paint);
        paint.setColor(-1);
        if (elevhelm) {
            canvas.drawText("HELM quantum & Exponential curve", (float) (f1w / 2), (float) (((f0h * 4) / 5) + 20), paint);
            if (RunSurfaceView.helmFlag == 0) {
                ...
                canvas.drawText("ELEV", (float) (f1w - w0), ((float) ((f0h / 2) + RunSurfaceView.arrow)) - (15.0f * scales), paint);
                j = 0;
                for (i = -VALID_MAX; i <= VALID_MAX; i++) {
                    j = i;
                    k = CountExp(j);
                    if (i % fix == 0) {
                        canvas.drawPoint((float) ((f1w - w0) + (j / fix)), (float) (((f0h + RunSurfaceView.arrow) / 2) - (k / fix)), paint);
                    }
                }
                ...
                canvas.drawText("AILE", (float) (f1w - w0), ((float) ((f0h / 2) + RunSurfaceView.arrow)) - (15.0f * scales), paint);
                j = 0;
                for (i = -VALID_MAX; i <= VALID_MAX; i++) {
                    j = i;
                    k = CountExp(j);
                ...
                canvas.drawText("RUDD", (float) (f1w - w0), ((float) ((f0h / 2) + RunSurfaceView.arrow)) - (15.0f * scales), paint);
                j = 0;
                for (i = -VALID_MAX; i <= VALID_MAX; i++) {
                    j = i;
                    k = CountExp(j);
            ...
            if (throhelm) {
                ...
                if (count == 0) {
                    expOn = false;
                } else if (RunSurfaceView.expFlag) {
                    expOn = false;
                } else {
                    expOn = true;
                }
                for (i = -VALID_MAX; i <= VALID_MAX; i++) {
                    k = GetThroCurveValue(i, RunSurfaceView.throIDec * 2, ch_en, ch_out, expOn);
                    if (i % fix == 0) {
                        canvas.drawPoint((float) ((f1w - w0) + (i / fix)), (float) (((f0h + RunSurfaceView.arrow) / 2) - (k / fix)), paint);
                    }
                }
                canvas.drawText(Float.toString(RunSurfaceView.adjustratio), (float) (f1w - w0), ((float) ((f0h / 2) + RunSurfaceView.arrow)) - (15.0f * scales), paint);
                canvas.drawText("Throttle curve", (float) (f1w / 2), (float) (((f0h * 4) / 5) + 20), paint);
            }
            if (screwpitch) {
                count = 0;
                ch_ens[0] = RunSurfaceView.pointLSFlag;
                ...
                if (count == 0) {
                    expSOn = false;
                } else if (RunSurfaceView.expSFlag) {
                    expSOn = false;
                } else {
                    expSOn = true;
                }
                for (i = -VALID_MAX; i <= VALID_MAX; i++) {
                    k = GetThroCurveValue(i, 0, ch_ens, ch_outs, expSOn);
                    if (i % fix == 0) {
                        canvas.drawPoint((float) ((f1w - w0) + (i / fix)), (float) (((f0h + RunSurfaceView.arrow) / 2) - (k / fix)), paint);
                    }
                }
                canvas.drawText(Float.toString(RunSurfaceView.adjustratio), (float) (f1w - w0), ((float) ((f0h / 2) + RunSurfaceView.arrow)) - (15.0f * scales), paint);
                canvas.drawText("Screw Pitch(PIT)curve", (float) (f1w / 2), (float) (((f0h * 4) / 5) + 20), paint);
            }
        }
        j = 0;
        if (throhelm) {
