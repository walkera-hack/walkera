...
public class EncodeDisplay {
    ...
    /* renamed from: com.walkera.wifia.EncodeDisplay.1 */
    ...
    public Bitmap BitmapFit(Bitmap bitmap) {
        ...
        if (IsFlag) {
            if (ThemeSurfaceView.f15w >= bmpWidth) {
                wRatio = (((float) ThemeSurfaceView.f15w) * 1.0f) / ((float) bmpWidth);
            } else {
                wRatio = (((float) bmpWidth) * 1.0f) / ((float) ThemeSurfaceView.f15w);
            }
            if (ThemeSurfaceView.f14h >= bmpHeight) {
                hRatio = (((float) ThemeSurfaceView.f14h) * 1.0f) / ((float) bmpHeight);
            } else {
                hRatio = (((float) bmpHeight) * 1.0f) / ((float) ThemeSurfaceView.f14h);
            }
            IsFlag = false;
        }
        matrix.reset();
    ...
    public String getDate() {
        ...
        int milisecond = ca.get(Calendar.MILLISECOND);
        return "" + year + (month + 1) + day + hour + minute + second + milisecond;
    }
    ...
    private String getPhotoFileName() {
        return new StringBuilder(String.valueOf(new SimpleDateFormat("'IMG'_yyyy-MM-dd HH:mm:ss").format(new Date(System.currentTimeMillis())))).append(".jpg").toString();
    }
